...
from accelerate.utils import imports as accelerate_imports
...
def load_local_checkpoint(ckpt_path: pathlib.Path) -> Dict[str, Any]:
    if ckpt_path.is_dir():
    ...
    prefix_candidates = ("model.", "module.", "transformer.")
    prefix_counts = {p: sum(k.startswith(p) for k in state.keys()) for p in prefix_candidates}
    use_prefix = None
    if any(prefix_counts.values()):
        use_prefix = max(prefix_counts, key=prefix_counts.get)

    if use_prefix:
        state = {k: v for k, v in state.items() if k.startswith(use_prefix)}

    return {_strip_known_prefixes(k): v for k, v in state.items()}
...
def convert_transformer(model_type: str, stage: str = None, ckpt_path: str | None = None):
    config, RENAME_DICT, SPECIAL_KEYS_REMAP = get_transformer_config(model_type)

    diffusers_config = config["diffusers_config"]
    model_id = config["model_id"]

    if ckpt_path is not None and stage is not None:
        raise ValueError("Local ckpt conversion does not support staged (Wan2.2) weights.")

    if ckpt_path is not None:
        original_state_dict = load_local_checkpoint(pathlib.Path(ckpt_path))
    else:
    ...
    with init_empty_weights():
        if "Animate" in model_type:
            if WanAnimateTransformer3DModel is None:
                raise ImportError("WanAnimateTransformer3DModel not available in installed diffusers.")
            transformer = WanAnimateTransformer3DModel.from_config(diffusers_config)
        elif "VACE" in model_type:
            if WanVACETransformer3DModel is None:
                raise ImportError("WanVACETransformer3DModel not available in installed diffusers.")
    ...
    expected_state = transformer.state_dict()
    missing_keys = [k for k in expected_state.keys() if k not in original_state_dict]
    unexpected_keys = [k for k in original_state_dict.keys() if k not in expected_state]
    mismatched_shapes = [
        (k, tuple(original_state_dict[k].shape), tuple(expected_state[k].shape))
        for k in original_state_dict.keys()
        if k in expected_state and original_state_dict[k].shape != expected_state[k].shape
    ]

    if mismatched_shapes:
        examples = ", ".join([f"{k}: {src} vs {dst}" for k, src, dst in mismatched_shapes[:5]])
        raise ValueError(f"Shape mismatches ({len(mismatched_shapes)}). Examples: {examples}")
    if missing_keys:
        examples = ", ".join(missing_keys[:10])
        raise ValueError(f"Missing keys after mapping ({len(missing_keys)}). Examples: {examples}")
    if unexpected_keys:
        print(f"Dropping {len(unexpected_keys)} unexpected keys not used by the diffusers model.")
...
def get_args():
    ...
    parser.add_argument("--ckpt_path", type=str, default=None, help="Local ckpt/safetensors path for transformer.")
    parser.add_argument("--base_model", type=str, default=None, help="Local diffusers base model to copy components.")
...
if __name__ == "__main__":
    ...
    dtype = None if args.dtype == "none" else DTYPE_MAPPING[args.dtype]
    dtype_kw = {} if dtype is None else {"torch_dtype": dtype}

    if "Wan2.2" in args.model_type and "TI2V" not in args.model_type and "Animate" not in args.model_type:
        transformer = convert_transformer(args.model_type, stage="high_noise_model", ckpt_path=args.ckpt_path)
        transformer_2 = convert_transformer(args.model_type, stage="low_noise_model", ckpt_path=args.ckpt_path)
    else:
        transformer = convert_transformer(args.model_type, ckpt_path=args.ckpt_path)
        transformer_2 = None

    if args.base_model:
        vae = AutoencoderKLWan.from_pretrained(args.base_model, subfolder="vae", **dtype_kw)
        text_encoder = UMT5EncoderModel.from_pretrained(args.base_model, subfolder="text_encoder", **dtype_kw)
    ...
    if "Wan2.2" in args.model_type and "I2V" in args.model_type and "TI2V" not in args.model_type:
    ...
    elif "Wan2.2" in args.model_type and "T2V" in args.model_type:
        pipe = WanPipeline(
            transformer=transformer,
            transformer_2=transformer_2,
            text_encoder=text_encoder,
    ...
    elif "Wan2.2" in args.model_type and "TI2V" in args.model_type:
        pipe = WanPipeline(
            transformer=transformer,
            text_encoder=text_encoder,
            tokenizer=tokenizer,
        ...
        if args.base_model:
            image_encoder = CLIPVisionModelWithProjection.from_pretrained(
                args.base_model, subfolder="image_encoder", **dtype_kw
            )
            image_processor = CLIPImageProcessor.from_pretrained(args.base_model, subfolder="image_processor")
        else:
            image_encoder = CLIPVisionModelWithProjection.from_pretrained(
        ...
        if WanVACEPipeline is None:
            raise ImportError("WanVACEPipeline not available in installed diffusers.")
        ...
        if WanAnimatePipeline is None:
            raise ImportError("WanAnimatePipeline not available in installed diffusers.")
        if args.base_model:
            image_encoder = CLIPVisionModel.from_pretrained(args.base_model, subfolder="image_encoder", **dtype_kw)
            image_processor = CLIPImageProcessor.from_pretrained(args.base_model, subfolder="image_processor")
        else:
